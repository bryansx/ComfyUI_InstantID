...
class InstantID(torch.nn.Module):
    def __init__(self, instantid_model, cross_attention_dim=1280, output_cross_attention_dim=1024,
...
class InstantIDFaceAnalysis:
    ...
    def load_insight_face(self, provider):
        model = FaceAnalysis(name="antelopev2", root=INSIGHTFACE_DIR,
                             providers=[provider + 'ExecutionProvider', ])  # alternative to buffalo_l
...
class ApplyInstantIDFromEmbed:
    ...
    def apply_instantid(self, instantid, face_embeds, control_net, model, positive, negative, start_at, end_at,
                        weight=.8, ip_weight=None, cn_strength=None, noise=0.35, image_kps=None, mask=None,
        ...
        face_embeds = ast.literal_eval(face_embeds)  # Converts string to list
        face_embeds = [torch.from_numpy(np.array(f)).unsqueeze(0) for f in face_embeds]
        ...
        clip_embed = face_embeds
        # InstantID works better with averaged embeds
        ...
        for conditioning in [positive, negative]:
            ...
            for t in conditioning:
                ...
                d['cross_attn_controlnet'] = image_prompt_embeds.to(comfy.model_management.intermediate_device(),
                                                                    dtype=c_net.cond_hint_original.dtype) if is_cond else uncond_image_prompt_embeds.to(
                    comfy.model_management.intermediate_device(), dtype=c_net.cond_hint_original.dtype)
...
class ApplyInstantID:
    ...
    def apply_instantid(self, instantid, insightface, control_net, image, model, positive, negative, start_at, end_at,
                        weight=.8, ip_weight=None, cn_strength=None, noise=0.35, image_kps=None, mask=None,
                        combine_embeds='average'):
        dtype = comfy.model_management.unet_dtype()
        if dtype not in [torch.float32, torch.float16, torch.bfloat16]:
        ...
        face_kps = extractFeatures(insightface, image_kps if image_kps is not None else image[0].unsqueeze(0),
                                   extract_kps=True)
...
class ApplyInstantIDControlNet:
    ...
    def apply_controlnet(self, face_embeds, control_net, image_kps, positive, negative, strength, start_at, end_at,
                         mask=None):
        self.device = comfy.model_management.get_torch_device()
        ...
        for conditioning in [positive, negative]:
            ...
            for t in conditioning:
                ...
                d['cross_attn_controlnet'] = image_prompt_embeds.to(
                    comfy.model_management.intermediate_device()) if is_cond else uncond_image_prompt_embeds.to(
...
NODE_DISPLAY_NAME_MAPPINGS = {
    "ApplyInstantIDFromEmbed": "Apply InstantID From Embed",
